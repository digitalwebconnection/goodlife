        ...
        <div className="grid lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-8">
            ...
            >
              ...
              <h1 className="font-serif font-black text-white leading-[1.05] tracking-tight mb-6 text-[clamp(2.5rem,6vw,5rem)] drop-shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
                Transform Your<br />
                Health, <span className=" text-[#FBAD18]">Naturally.</span>
              </h1>

              <div className="flex items-center gap-4 mb-8 pl-4 border-l-2 border-brand-orange">
                <h2 className="text-lg sm:text-xl font-light text-white/90 leading-relaxed italic">
                  Weight Loss that Heals, <span className="font-bold text-white not-italic underline decoration-brand-orange underline-offset-8">NOT Hurts</span>
                </h2>
              </div>
              ...
              <div className="flex flex-wrap gap-5">
                <motion.a
                  ...
                  whileTap={{ scale: 0.95 }}
                  href="#plans"
                  className="px-12 py-2 rounded-full bg-brand-amber text-black font-black flex items-center gap-3 text-lg uppercase tracking-widest shadow-xl shadow-brand-amber/20"
                >
                  Explore Plans
                </motion.a>
                <motion.a
                  ...
                  href="#contact"
                  className="px-12 py-2 rounded-full border-2 border-white/40 text-white font-black text-lg uppercase tracking-widest backdrop-blur-sm transition-all"
                >
                  Get Started
                </motion.a>
              </div>
            ...
            <div className="grid gap-4">
              {STATS.map((stat, i) => (
                <motion.div
                  ...
                  className="p-6 rounded-3xl bg-white/25 border border-white/10 backdrop-blur-xl"
                >
                  <h4 className="text-3xl font-black text-brand-teal mb-1">{stat.value}</h4>
                  <p className="text-black/90 text-xs font-bold uppercase tracking-widest">{stat.label}</p>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <div className="flex gap-2">
          {HERO_IMAGES.map((_, index) => (
            <button
              ...
              className={`h-1.5 rounded-full transition-all duration-700 ${index === activeIndex ? 'w-12 bg-brand-amber' : 'w-4 bg-white/20 hover:bg-white/40'}`}
            />
          ))}
        </div>
      </div>
