  ...
  return (
    <>
          ...
          <nav className="hidden lg:flex items-center gap-8">
            {NAV_LINKS.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className="text-[11px] font-black uppercase tracking-[0.2em] transition-all duration-500 hover:-translate-y-[2px] relative group text-white/90 hover:text-[#FFC300]"
              >
                {link.label}
                <span className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-[#FFC300] rounded-full opacity-0 scale-0 group-hover:opacity-100 group-hover:scale-100 transition-all duration-500" />
              </a>
            ))}
            {/* Desktop CTA */}
            <div className="hidden lg:flex items-center gap-6">
              <a
                ...
                className="ripple-btn shimmer-hover px-4 py-3 rounded-full text-xs font-black uppercase tracking-widest transition-all duration-500 flex items-center gap-3 shadow-lg bg-[#FFC300] text-brand-black shadow-[#FFC300]/20"
              >
                Consult Now
      ...
      >
        ...
        <div className="absolute top-0 right-0 w-[80%] h-[80%] bg-[#FFC300]/10 blur-[120px] rounded-full pointer-events-none -translate-y-1/2 translate-x-1/2" />
        ...
        <div className="container-custom h-full flex flex-col items-center justify-center relative z-10">
          <nav className="flex flex-col items-center gap-10">
            {NAV_LINKS.map((link, i) => (
              <a
                ...
                className={`text-4xl sm:text-5xl font-black font-playfair text-brand-black hover:text-[#FFC300] transition-all duration-500 transform ${menuOpen ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
                  }`}
          ...
          <div
            className={`mt-16 flex flex-col items-center gap-6 transition-all duration-700 delay-[1000ms] ${menuOpen ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
              }`}
          >
            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-400">Get in touch</p>
            <a href="tel:9979740361" className="text-2xl font-bold text-brand-black font-playfair tracking-tight hover:text-[#FFC300] transition-colors">
              +91 9979740361
            </a>
            <div className="flex gap-6 mt-4">
              {['📸', '▶️', '💬'].map((icon, idx) => (
                <div key={idx} className="w-12 h-12 rounded-full border border-gray-100 flex items-center justify-center text-xl bg-white shadow-sm hover:border-[#FFC300] transition-all cursor-pointer">
                  {icon}
                </div>
              ))}
            </div>
