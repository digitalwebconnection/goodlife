export default function CTABanner() {
  return (
    <section className="relative py-24 lg:py-32 overflow-hidden bg-brand-black">
      ...
      <div className="container-custom relative z-10">
        <div 
          data-aos="zoom-in"
          className="max-w-5xl mx-auto bg-white/5 backdrop-blur-2xl border border-white/10 rounded-[4rem] p-12 lg:p-24 text-center relative overflow-hidden group shadow-[0_50px_150px_rgba(0,0,0,0.5)]"
        >
          {/* Animated Glows */}
          ...
          <div className="relative z-10">
            ...
            <h2 
              data-aos="fade-up" 
              data-aos-delay="300"
              className="text-white text-5xl lg:text-7xl font-black font-playfair leading-[1.05] mb-12 tracking-tight"
            >
              Ready to Start Your<br />
              <span className="italic font-normal text-brand-yellow">Success Story?</span>
            </h2>
            
            <p 
              data-aos="fade-up" 
              data-aos-delay="400"
              className="text-white/40 text-lg lg:text-xl leading-relaxed max-w-2xl mx-auto mb-16"
            >
            ...
            >
              <a
                ...
                className="ripple-btn group w-full sm:w-auto bg-brand-yellow hover:bg-white text-brand-black font-black px-12 py-6 rounded-full text-sm uppercase tracking-[0.2em] transition-all duration-700 shadow-[0_20px_50px_rgba(255,193,7,0.2)] flex items-center justify-center gap-4 hover:scale-110"
              >
              ...
              <div className="flex -space-x-4">
                {[1,2,3,4].map(i => (
                  <div key={i} className="w-12 h-12 rounded-full border-2 border-brand-black bg-brand-dark shadow-xl transition-transform hover:-translate-y-2" />
                ))}
                <div className="w-12 h-12 rounded-full border-2 border-brand-black bg-white/10 backdrop-blur-md flex items-center justify-center text-[10px] font-black text-white">
                  +20k
                </div>
              </div>
            </div>

            <div className="mt-20 pt-12 border-t border-white/5 flex flex-wrap justify-center gap-x-12 gap-y-6 opacity-30">
              {['No Pills', 'No Gym', 'No Side Effects', 'Home Based'].map(t => (
                <div key={t} className="flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-brand-yellow" />
                  <span className="text-[10px] font-black uppercase tracking-[0.3em] text-white">{t}</span>
                </div>
              ))}
            </div>
          </div>
